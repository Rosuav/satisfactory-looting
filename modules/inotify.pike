	...
	while (sscanf(text, "%s\xA3%s%[ .,()\xA3]%s", string plain, string icon, string end, text) == 4) {
		...
		if (arrayp(img)) {
			...
			foreach (img, string fn) {
				object|mapping png = G->G->parser->load_image(EU4_PROGRAM_PATH + "/" + fn);
...
void watch_game_log(object inot) {
	...
	string logfn = EU4_LOCAL_PATH + "/logs/game.log";
	...
	void parse() {
		...
		while (sscanf(data, "%s\n%s", string line, data)) {
			...
			if (sizeof(sendme) > 1) G->G->websocket_types->tag->send_to_all(sendme);
		}
	}
	parse();
	int pos = log->tell();
