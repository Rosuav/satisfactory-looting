...
constant http_path_pattern = "/flags/%[A-Z_a-z0-9]%[-0-9A-F].png";

mapping(string:mixed) http_request(Protocols.HTTP.Server.Request req, string tag, string color) {
	if (tag == "") return 0;
	...
	if (tag == "Custom") {
		...
		[Image.Image backgrounds, int bghash] = G->G->parser->load_image(EU4_PROGRAM_PATH + "/gfx/custom_flags/pattern" + "2" * (flag >= 34) + ".tga", 1);
		...
		[mapping symbols, int symhash] = G->G->parser->load_image(EU4_PROGRAM_PATH + "/gfx/interface/client_state_symbols_large.dds", 1);
		...
		[img, int hash] = G->G->parser->load_image(EU4_PROGRAM_PATH + "/gfx/flags/" + tag + ".tga", 1);
		if (!img) return 0;
